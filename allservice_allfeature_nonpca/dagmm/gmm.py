...
class GMM:
    ...
    def fit(self, z, gamma):
        ...
        with tf.compat.v1.variable_scope("GMM"):
            ...
            self.L = tf.compat.v1.cholesky(sigma + min_vals[None,:,:])
    ...
    def energy(self, z):
        ...
        with tf.compat.v1.variable_scope("GMM_energy"):
            ...
            v = tf.compat.v1.matrix_triangular_solve(self.L, tf.transpose(z_centered, [1, 2, 0]))  # kli

            # log(det(Sigma)) = 2 * sum[log(diag(L))]
            log_det_sigma = 2.0 * tf.compat.v1.reduce_sum(tf.compat.v1.log(tf.compat.v1.matrix_diag_part(self.L)), axis=1)

            # To calculate energies, use "log-sum-exp" (different from orginal paper)
            d = z.get_shape().as_list()[1]
            logits =tf.compat.v1.log(self.phi[:,None]) - 0.5 * (tf.compat.v1.reduce_sum(tf.compat.v1.square(v), axis=1)
                + d * tf.compat.v1.log(2.0 * np.pi) + log_det_sigma[:,None])
            energies = - tf.compat.v1.reduce_logsumexp(logits, axis=0)

        return energies

    def cov_diag_loss(self):
        with tf.compat.v1.variable_scope("GMM_diag_loss"):
            diag_loss = tf.compat.v1.reduce_sum(tf.divide(1, tf.compat.v1.matrix_diag_part(self.sigma)))
